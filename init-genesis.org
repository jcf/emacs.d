...
** Sensible window splitting
   ...
   As we always wrap to 80 columns we drop ~split-width-threshold~ from the
   default of ~160~ to ~85~, and set the height threshold to ~nil~ to persuade
   Emacs to use vertical splits more often than not.
